...
def compute_metrics(
    ...
    strict_malicious_contained_rows = 0
    benign_rows = 0
    benign_restricted_rows = 0
    expected_tier_rows = 0
    expected_tier_hit_rows = 0
    security_expected_tier_rows = 0
    security_expected_tier_hit_rows = 0
    under_containment_rows = 0
    over_containment_rows = 0
    ...
    for row in rows:
        ...
        active_order = TIER_ORDER.get(active_tier, 1)
        expected_order = TIER_ORDER.get(expected_tier, 1)
        if expected_tier in TIER_ORDER:
            expected_tier_rows += 1
            if active_order >= expected_order:
                expected_tier_hit_rows += 1
            if workload_class in {"suspicious", "malicious"}:
                security_expected_tier_rows += 1
                if active_order >= expected_order:
                    security_expected_tier_hit_rows += 1
                if active_order < expected_order:
                    under_containment_rows += 1
            if active_order > expected_order:
                over_containment_rows += 1

        if container_id not in first_timestamp:
            first_timestamp[container_id] = timestamp
        if (
            container_id not in first_expected_hit
            and active_order >= expected_order
        ):
        ...
        if workload_class == "malicious":
            ...
            if active_order >= expected_order:
                strict_malicious_contained_rows += 1
    ...
    return {
        ...
        "strict_malicious_containment_rate": _safe_divide(strict_malicious_contained_rows, malicious_rows),
        "expected_tier_hit_rate": _safe_divide(expected_tier_hit_rows, expected_tier_rows),
        "security_expected_tier_hit_rate": _safe_divide(
            security_expected_tier_hit_rows,
            security_expected_tier_rows,
        ),
        "under_containment_rate": _safe_divide(under_containment_rows, security_expected_tier_rows),
        "over_containment_rate": _safe_divide(over_containment_rows, expected_tier_rows),
        "under_containment_events": under_containment_rows,
        "over_containment_events": over_containment_rows,
        "tier_occupancy": {
