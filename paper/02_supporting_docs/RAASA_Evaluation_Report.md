...
If an agent needs to compile a heavy binary, a static secure sandbox (L3) may impose unnecessary friction. However, giving an agent wide-open access (L1) can increase the blast radius of a destructive shell attempt or runaway loop. RAASA's integration addresses this bounded trade-off in the tested scenarios: the agent can begin in a lightweight tier, while telemetry-driven thresholds can move the workload toward L3 when its execution profile shifts from expected build activity to measured anomaly.
...
The implementation provides evidence that automated, bounded containment decisions derived from runtime container telemetry can improve selected security-utility trade-offs relative to fixed allocations.
