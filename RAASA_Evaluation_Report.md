...
- **Enforcement Mechanism**: L3 network containment is implemented through
  Linux `tc` profiles on pod-specific host interfaces. Current validated L3
  behavior is hard containment, not a general-purpose sandbox boundary.
- **Verdict**: **Strong single-node prototype evidence.** The adaptive paradigm
  carried from local Docker into AWS/K3s, but this does not establish
  production readiness, multi-node robustness, or protection against container
  escape.
...
If an agent needs to compile a heavy binary, a static strict sandbox can break
legitimate work. However, giving an agent wide-open access means runaway or
destructive behavior can damage the host. RAASA does not solve this entirely;
it provides a bounded adaptive-control layer that can move a workload toward
L2/L3 when telemetry shows risky behavior.
...
The implementation supports the narrower paper claim that automated, bounded
containment decisions derived from runtime telemetry can improve the
security-utility trade-off over fixed L1/L3 allocation within the tested scope.
